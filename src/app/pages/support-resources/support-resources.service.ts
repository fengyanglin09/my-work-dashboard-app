...
import { CheatItem, ResourceCategory } from '../../core/model/cheatsheet.model';

const noAction = () => {};

const link = (label: string, url: string): CheatItem => ({
    label,
    type: 'link',
    icon: 'fas fa-link',
    url,
...
const download = (label: string, filePath: string): CheatItem => ({
    label,
    type: 'download',
...
const doc = (label: string, mdFilePath: string): CheatItem => ({
    label,
    type: 'doc',
